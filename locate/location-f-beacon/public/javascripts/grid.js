...
function getAvgDis(i){
    ...
    return avg_d[i].value;
}

function getKeyName(i){
    console.log(`avg_d[i] : ${avg_d[i].key} : ${i}`);
    return avg_d[i].key;
}
    ...
    fetch('/getStatus').then(res => res.json()).then(
        r => {
            ...
            for(let i = 0; i < r.data.length; i++){
                switch(i){
                    case 0:
                        document.getElementById('chart-div-value1').textContent = '';
                        let target_div = document.getElementById('chart-div-value1');
                        let target_html =  `<ul class="child-status">`;
                        target_html += `<li>ID: ${r.data[i][0].key}</li>`;
                        avg_d[i].key = r.data[i][0].key;
                        let dist = [];
                        ...
                        avg_d[i].value = avg_disc.toFixed(2);
                        ...
                        break;
                    case 1:
                        document.getElementById('chart-div-value2').textContent = '';
                        let target_div1 = document.getElementById('chart-div-value2');
                        let target_html1 =  `<ul class="child-status">`;
                        target_html1 += `<li>ID: ${r.data[i][0].key}</li>`;
                        avg_d[i].key = r.data[i][0].key;
                        let dist1 = [];
                        for(let row of r.data[i]){
                            dist1.push(Number(row.distance));
                        }
                        let avg_disc1 = average(dist1);
                        target_html1 += `<li class="newline">AVG Distance: ${avg_disc1.toFixed(2)}</li>`;
                        avg_d[i].value = avg_disc1.toFixed(2);
                
                        for(let row of r.data[i]){
                            //avg_d.push(Number(row.distance).toFixed(2));
                            let avg = Number(row.distance).toFixed(2);
                            target_html1 += `<li>distance: ${avg}</li>`;
                        }
                        target_html1 += `</ul>`;
                        target_div1.insertAdjacentHTML('afterbegin', target_html1);
                        
                        break;
                    case 2:
                        document.getElementById('chart-div-value3').textContent = '';
                        let target_div2 = document.getElementById('chart-div-value3');
                        let target_html2 =  `<ul class="child-status">`;
                        target_html2 += `<li>ID: ${r.data[i][0].key}</li>`;
                        avg_d[i].key = r.data[i][0].key;
                        let dist2 = [];
                        for(let row of r.data[i]){
                            dist2.push(Number(row.distance));
                        }
                        let avg_disc2 = average(dist2);
                        target_html2 += `<li class="newline">AVG Distance: ${avg_disc2.toFixed(2)}</li>`;
                        avg_d[i].value = avg_disc2.toFixed(2);
                
                        for(let row of r.data[i]){
                            //avg_d.push(Number(row.distance).toFixed(2));
                            let avg = Number(row.distance).toFixed(2);
                            target_html2 += `<li>distance: ${avg}</li>`;
                        }
                        target_html2 += `</ul>`;
                        target_div2.insertAdjacentHTML('afterbegin', target_html2);
                        break;
                }
            }
        });
